# Organization Section

This section covers all pages related to organization management in CKAN.

## Overview

Organizations in CKAN are used to:

- Group datasets under ownership
- Manage user permissions
- Control dataset visibility
- Organize content hierarchically

## Pages in This Section

| Page                                      | Description                    |
|-------------------------------------------|--------------------------------|
| [Index](search.md)                         | List all organizations         |
| [Read](read.md)                           | View organization details      |
| [About](about.md)                         | Organization about page        |
| [Create](new.md)                          | Create new organization        |
| [Edit](edit.md)                           | Edit organization              |
| [Members](members.md)                     | View members                   |
| [Add Member](member-new.md)               | Add member                     |
| [Manage Members](manage-members.md)       | Manage members                 |
| [Administrators](admins.md)               | View admins                    |
| [Activity](activity.md)                   | Activity stream                |
| [Bulk Process](bulk-process.md)           | Bulk dataset management        |

## Related Sections

- [Dataset](../dataset/index.md) - Owned datasets
- [Group](../group/index.md) - Thematic groups
- [User](../user/index.md) - Organization members
