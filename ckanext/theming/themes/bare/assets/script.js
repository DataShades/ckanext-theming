...
((ckan) => {
    ...
    const ui = {
        ...
        autocomplete(element, params = {}) {
            ...
            if (params.source)
                el.setAttribute("data-module-source", params.source);
            if (params.options)
                el.setAttribute("data-module-options", JSON.stringify(params.options));
            if (params.allowMultiple !== undefined)
                el.setAttribute("data-module-allow-multiple", String(params.allowMultiple));
            if (params.allowNew !== undefined)
                el.setAttribute("data-module-allow-new", String(params.allowNew));
            if (params.selected)
                el.setAttribute("data-module-selected", JSON.stringify(params.selected));
            if (params.idKey)
                el.setAttribute("data-module-id-key", params.idKey);
            if (params.labelKey)
                el.setAttribute("data-module-label-key", params.labelKey);
            if (params.joined !== undefined)
                el.setAttribute("data-module-joined", String(params.joined));
            if (params.separator)
                el.setAttribute("data-module-separator", params.separator);
            if (params.minChars !== undefined)
                el.setAttribute("data-module-min-chars", String(params.minChars));
            if (params.debounce !== undefined)
                el.setAttribute("data-module-debounce", String(params.debounce));
            // @ts-ignore
        ...
        getAutocomplete(id_or_el) {
            const el = typeof id_or_el === "string"
                ? (document.getElementById(id_or_el) || document.querySelector(id_or_el))
                : id_or_el;
            if (!el) {
                return null;
            }
            ...
            const instance = instances.find((inst) => inst.el[0] === el);
            if (instance) {
    ...
    class Autocomplete {
        ...
        onchange(callback) {
            const handler = (e) => {
                const values = e.detail.values.map((v) => v.value);
                callback(values);
            };
