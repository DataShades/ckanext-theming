 ...
 *
 * Works on an <input> element carrying [data-theming-module="autocomplete"].
 ...
 *
 *   data-autocomplete-source         URL for ajax mode.  Query spliced at
 ...
 *
 *   data-autocomplete-selected       JSON pre-selection.
 *
 ...
 *   data-autocomplete-min-chars      Min chars before suggestions. Default: 1/2
...
class Autocomplete {
  ...
  constructor(element) {
    this.root = element
    this.combobox = element // Always use the root input as the visible combobox
    ...
    this.minChars  = parseInt(
      this.root.dataset.autocompleteMinChars ?? (this.source ? '2' : '0'), 10
    )
    this.debounce = parseInt(this.root.dataset.autocompleteDebounce ?? '300', 10)

    // Static options from data attr
    this._staticOptions = []
    if (!this.source) {
      try {
        this._staticOptions = JSON.parse(this.root.dataset.autocompleteOptions || '[]')
    ...
    // Pre-selected items from data attr
    ...
    this._debouncedFetch = Autocomplete._debounce(this._fetchOptions.bind(this), this.debounce)
  }
  ...
  init() {
    ...
    if (!this.selections) {
      console.warn(`Autocomplete: container #${this.containerId} not found`, this.root)
      return
    }
    ...
    this.listbox.setAttribute('aria-label',
  ...
  select(value, label) { this._addSelection(String(value), String(label)) }
  remove(value)        { this._removeSelection(String(value)) }
  get value()          { return [...this._selections] }
  ...
  _onFocus() {
    if (this.combobox.value.length >= this.minChars) this._suggest(this.combobox.value)
  }
  ...
  _onKeydown(e) {
    ...
    switch (key) {
      case 'ArrowDown':
        e.preventDefault()
        if (!this._open) { this._suggest(this.combobox.value); break }
        this._setActive(Math.min(this._activeIdx + 1, options.length - 1), options)
        break
      ...
      case 'Home':
        if (this._open) { e.preventDefault(); this._setActive(0, options) }
        break

      case 'End':
        if (this._open) { e.preventDefault(); this._setActive(options.length - 1, options) }
        break
      ...
      case 'Backspace':
        if (this.combobox.value === '' && this.isMultiple && this._selections.length > 0) {
          this._removeSelection(this._selections[this._selections.length - 1][this.idKey])
        }
        break
  ...
  _onDocClick(e) {
    if (!this.combobox.contains(e.target) && !this.listbox.contains(e.target)) this._close()
  }
  ...
  _suggest(query) {
    ...
    const matched = this._staticOptions.filter(opt => {
      ...
      const display = String(opt[this.labelKey] ?? opt[this.idKey] ?? '').toLowerCase()
  ...
  _addSelection(value, label, isNew = false) {
    if (this.isMultiple && this._selections.some(s => s[this.idKey] === value)) return
    ...
    this.combobox.dispatchEvent(new CustomEvent('autocomplete:select', {
      bubbles: true,
  ...
  _removeSelection(value) {
    ...
    if (this.isMultiple) {
      this.selections
        ...
        ?.remove()
      if (this._selections.length === 0) this.selections.setAttribute('hidden', '')
    } else {
      this.combobox.value = ''
    }
    ...
    this.combobox.dispatchEvent(new CustomEvent('autocomplete:remove', {
      bubbles: true,
  ...
  _dispatchChange() {
    this.combobox.dispatchEvent(new CustomEvent('autocomplete:change', {
      bubbles: true,
      detail:  { values: this._selections.map(s => ({ value: s[this.idKey], label: s[this.labelKey] })) },
    }))
  }
  ...
  _getStorageValue() {
    ...
    if (this.storage.tagName === 'SELECT') {
      return Array.from(this.storage.options).filter(o => o.selected).map(o => o.value).join(',')
    }
  ...
  _syncHiddenFields() {
    if (this.isJoined) {
      this.storage.value = this._selections.map(s => s[this.idKey]).join(this.separator)
      ...
      this.storage.value = this._selections.length ? this._selections[0][this.idKey] : ''
    }
  ...
  _close() {
    ...
    if (!this.isMultiple) {
      this.combobox.value = this._selections.length ? this._selections[0][this.labelKey] : ''
    } else {
      this.combobox.value = ''
    }
  }
  ...
  _visibleOptions() {
    return Array.from(
      this.listbox.querySelectorAll('[role="option"]:not([hidden]):not([aria-disabled="true"])')
    )
  }
