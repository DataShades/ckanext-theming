...
class Select2Autocomplete {
  ...
  constructor(element) {
    ...
    if (!this.$.fn.select2) throw new Error('Select2Autocomplete requires Select2')
  ...
  init() {
    ...
    // Resolve the dropdown parent — scopes Select2's panel to our container
    // so CSS variables and z-index are inherited correctly
    const container = this.containerId
      ? document.getElementById(this.containerId)
      : this.root.parentElement
    const $parent = container ? this.$(container) : this.$root.parent()
  ...
  remove(value) {
    if (!this.$root) return
    const current = this.$root.val() || []
    const next = Array.isArray(current) ? current.filter(v => v !== value) : ''
    this.$root.val(next).trigger('change')
  }
        ...
        processResults: (data) => {
          ...
          return {
            results: data.map(item =>
              typeof item === 'string'
                ? { id: item, text: item }
                : { id: item[this.idKey], text: item[this.labelKey] ?? item[this.idKey] }
            ),
          }
        },
        cache: true,
  ...
  _bindEvents() {
    this.$root.on('select2:select', (e) => {
      ...
      this.root.dispatchEvent(new CustomEvent('autocomplete:select', {
        bubbles: true,
        detail:  { value: String(value), label: String(label), isNew: !!isTag },
      }))
  ...
  _installJoinedShim() {
    ...
    this.$root.on('select2:select select2:unselect', () => {
      this._joinedInput.value = this.value.map(s => s.value).join(this.separator)
    })
  }
}
