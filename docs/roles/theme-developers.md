...
**API Completeness**: Implement all standard components defined in
[components.yaml](https://github.com/DataShades/ckanext-theming/tree/master/ckanext/theming/components.yaml)
to ensure any extension relying on `ui.*` components renders without throwing
errors.
...
extensions (such as `ckanext-spatial` or `ckanext-scheming`) are enabled.
