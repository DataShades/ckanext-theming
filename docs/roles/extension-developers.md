...
1. **Theme Portability**: Do not write raw HTML with framework-specific classes (e.g. Bootstrap's `btn btn-primary` or Tailwind's `bg-blue-500`).
2. **Semantic Calls**: Always call standard `ui.*` component macros (e.g., `{{ ui.button(...) }}`) in your extension templates.
3. **Graceful Fallbacks**: Ensure that if a portal uses an abstract or custom theme that doesn't natively support your extension, the layout does not break.
...
* **The Problem**: You want to alter a component's structure to match the look of a popular theme (e.g. `nsw-design-system`), but doing so with raw markup might break other themes.
...
* **The Goal**: Provide advanced layout controls that can't be automatically inserted without risking container layout issues.
* **The Method**: Provide a set of custom UI components in your extension, and document how the end user can call or incorporate them into their theme's templates. Provide code snippets and mockups rather than injecting raw tags programmatically.
